...
(function ($) {
	...
	// Active with Scroll animation
	if ($(".tj-hover-active-item").length > 0) {
		let mediaMatch = gsap.matchMedia();
		mediaMatch.add("(min-width: 1200px)", () => {
			const slider = document.querySelector(".h6-working-process");
			if (slider?.children?.length) {
				let panels = gsap.utils.toArray(".tj-hover-active-item");
				...
				gsap.to(panels, {
					...
					scrollTrigger: {
						...
						pin: true,
						scrub: 1,
						end: () => "+=" + 600, //Math.max(1000, slider.offsetWidth / 3),
						onUpdate: self => {
